								...
								dependencies: [
									{
										...
										"Functionality/APIs":
											"Google's Generative AI SDK for accessing Gemini models. Provides chat functionality, text generation, and model configuration.",
										...
										documentation: [
											{
												...
												explanation:
													"Essential for understanding how to set up and use the Gemini models, configure chat sessions, and handle responses in Node.js applications.",
											},
										],
									},
								],
	...
	try {
		...
		try {
			...
			if (!jsonMatch) {
				...
				const jsonCandidate = response.match(
					/\{\s*"dependencies"\s*:\s*\[[\s\S]*\]\s*\}/,
				);
				if (!jsonCandidate) {
			...
			try {
				...
				const fixedJson =
					extracted.replace(/,\s*$/, "") + // Remove trailing comma if present
